...
! local variables
logical twrite
integer ik,nmix,nwork,i,tra,zscl
real(8) dv,etp,de,timetot
! allocatable arrays
real(8), allocatable :: work(:),vb1(:),res(:)
...
!zst mix
!call scf_init(0.4,50,nmix,3)
call mix_init(mixsdb,nmix,vsbs,mixtype,broydpm(1),broydpm(2))
...
!  call mixerifc(mixtype,nmix,vsbs,dv,nwork,work)
  if (premix == 1 ) then
    if (tra == 0) then
      call pmix_scf(iscl,nmix,vsbs(:),dv,tra)
      if (tra > 0) then
        call mix_final
        call mix_init(mixsdb,nmix,vsbs,mixtype,broydpm(1),broydpm(2))
      end if
      ...
      zscl=iscl-tra
      call mix_scf(zscl,nmix,vsbs(:),dv)
    end if
  else if (premix == 2) then
    call mix_scf(iscl,nmix,vsbs(:),dv)
  else if (premix == 0) then
    call pmix_scf(iscl,nmix,vsbs(:),dv,tra)
  end if
